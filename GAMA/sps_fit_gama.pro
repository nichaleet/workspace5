 ...
 ;       if nloop eq 98 and keepoldfit eq 0 and curchisq-oldchisq gt 0.1 then maxnloop = 100
    ...
    ;calculate new signal to noise
       ...
       if c gt 0 then begin
          medres = median(abs(ts_diff(science.lambda,1)))/(1.+science.zfit)
          science.snfit = (1.0/mean(dev[w]))/sqrt(medres)
       endif 
    endif
    ;;;;;;;;;;;;;;;;;;;
...
pro sps_fit::statusbox, science=science
    ...
    widget_control, widget_info(self.base, find_by_uname='curid'), set_value=science.objname+'/'+' ('+strcompress(self.i+1, /rem)+' / '+strcompress(self.nspec, /rem)+')'
    ...
    widget_control, widget_info(self.base, find_by_uname='curmstar'), set_value=science.logmstar gt 0 ? strcompress(string(science.logmstar, format='(D10.2)'), /rem)+'+/-'+strcompress(string(science.logmstarerr, format='(D10.2)'), /rem) : unknown
...
pro sps_fit::getscience, files=files
    ...
    if ~file_test(sciencefits) then begin
        ...
        cat = mrdfits('/scr2/nichal/workspace5/GAMA/gama_cat_tofit.fits',1)
        ...
        for i=0,nspec-1 do begin
            ...
            data = mrdfits(files[i], 0,hdr,/silent)
               
            science.specid = sxpar(hdr,'SPECID')

            lambda = (findgen(npix)-sxpar(hdr,'crpix1')+1.)*$
                    sxpar(hdr,'CD1_1')+sxpar(hdr,'CRVAL1')
            spec   = data[*,0]
            ivar   = 1./(data[*,1])^2
	    ...
	    science.skyspec = data[*,4]
            ...
            ;cross match catalog
            cataid = sxpar(hdr,'CATAID')
            wmatch = where(cat.cataid eq cataid,cmatch)
            if cmatch ne 1 then stop,'something is wrong'
            catnow = cat(wmatch)
            science.objname = catnow.gama_name
            science.cataid = catnow.cataid
            science.zcat = catnow.z
            science.zspec = catnow.z 
            science.logmstar = catnow.logmstar
            science.logmstarerr = catnow.dellogmstar
            science.haew = catnow.ha_ew
            science.haewerr = catnow.ha_ew_err
            science.oiiew = catnow.oiib_ew
            science.oiiewerr = catnow.oiib_ew_err
...
pro sps_fit::initialize_directory, directory=directory
    common mask_in, mask_in, copynum
    newdirectory = '/scr2/nichal/workspace5/GAMA/sps_data/'+mask_in+'/'
    ...
    countfiles:
    files = file_search(directory, '*.{fit,fits,fits.gz}', count=c)
...
pro sps_fit_gama,mask=mask,copyi=copyi
    common mask_in, mask_in, copynum
    if ~keyword_set(mask) then mask = 'gama_test'
    mask_in = mask
    if ~keyword_set(copyi) then copyi=1
    copynum = strtrim(string(copyi,format='(I02)'),2)
    directory = '/scr2/nichal/workspace5/GAMA/data/spec'
